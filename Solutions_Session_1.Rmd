...
This notebook presents the solutions to exercises in Session 1


# Section 0:
## Comments `#`
In general, comments are used to insert code annotations explaining what a line of code does (hence the term comment). The `#` character is also used to prevent a line of code from running, this is called "commenting out" code.
In R the `#` comments out anything to the right of it, preventing that code from being run. Anything to the left of `#` will still run.
*note* I have made liberal use of `#`, particularly around package installation, so you don't reinstall a package everytime you run a script.
...
The following code generates a vector named xmple containing 10000 chisquare distributed random values. 
```{r}
BadCode  <-                                 rchisq(10000, 
                                                   10) 

hist(BadCode) 
abline(v=mean(BadCode), col = 'blue', 
       lwd = 2) 
abline(v = median(BadCode), 
       col = 'red', 
       lwd=2) 
```
Now explain why this is bad code. How readable is it? What should you do to change it?

Now rearrange each function call so that they are on a single line. Next annotate each line of code and explain what it does. 
```{r}
GoodCode  <-  rchisq(10000, 10) # Randomly Generate 10k chisquare distributed values, with df=10
hist(GoodCode) # Plots histogram
abline(v = mean(GoodCode), col = 'blue', lwd = 2) # Plots Mean line
abline(v = median(GoodCode), col = 'red', lwd = 2) # Plots Median line
```
...
In cases where you need only complete data, the missing value could be replaced, and excluded in later analyses.
...
I can also extract a sequence or "slice" from an object by using the colon `:` inside the square brackets:
```{r}
f_vec[1:2]
```
...
```{r}
mp1 <- c(1,18,12,23,8,4)
mp[mp1]
```

```{r}
mp2  <- c(7,18, 5, 29, 30, 31, 32, 33, 34)
mp[mp2]
```
...
Clearly this is a large data frame. So if we only had one or two variables that we were interested in, we could just select those columns using list inside square brackets.
...
print(dfSummary(data.e1.b, style = 'grid'), method = 'render') # text output
```
...
Import the "big5.csv" file from the data folder and store it in an object called big5
...
```{r}
print(dfSummary(big5_2, style = 'grid'), method = 'render') # produces text summary in console
```

If you are using an R script you can yse the following code to see the summary in the viewer window
```{r}
